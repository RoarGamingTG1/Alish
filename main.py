...
async def send_key(update):
    ...
    if user in user_keys.values():
        ...
        await update.message.reply_text(f"Your key is: {existing_key}")
    else:
        ...
        if await Bot.get_chat_member(int(CHANNEL_ID), user):
            ...
            await update.message.reply_text(f"Your key is: {key}")
        else:
            ...
            await update.message.reply_text(join_channel_message, disable_web_page_preview=True)
...
async def verify_user(update):
    ...
    if await Bot.get_chat_member(int(CHANNEL_ID), user):
        ...
        await update.message.reply_text("You haven't joined our channel yet. Please join our channel [here](https://t.me/QTVinfo) and click 'Verify' again.", disable_web_page_preview=True)

# Function to check user's own status
async def check_user_status(update):
    user = update.from_user.id
    if await Bot.get_chat_member(int(CHANNEL_ID), user):
        await update.message.reply_text("You have joined our channel.")
    else:
        await update.message.reply_text("You have not joined our channel yet.")

# Function to check total keys issued
async def check_total_keys(update):
    global total_keys_issued
    await update.message.reply_text(f"Total keys issued: {total_keys_issued}")
